...
function Billboard:Order(rad) --this checks whether the object is within render distance, and if it's behind the camera
    self.rad = rad
    self.rotz = (self.zpos - self.camera.zpos) * math.cos(rad) + (self.xpos - self.camera.xpos) * math.sin(rad) + self.camera.zpos -- rotating the z point around the y axis
...
function Billboard:CheckZ(rad) -- function to check whether the object is within the drawspace
    ...
    self.rotx = (self.xpos - self.camera.xpos) * math.cos(rad) - (self.zpos - self.camera.zpos) * math.sin(rad) + self.camera.xpos -- rotating the x point around the y axis
    self.drawx = -200 * (self.rotx - self.camera.xpos) / math.abs(self.rotz - self.camera.zpos) -- using perspective math to calculate draw point on draw plane
...
function Billboard:ShowSelf()
    ...
    self.drawy = 200 * (self.roty - self.camera.ypos) / math.abs(self.rotz - self.camera.zpos) -- using perspective math to calculate draw point on draw plane
    self.drawz = self.rotz
    self.drawSizeX = (200 * (self.rotx + self.w - self.camera.xpos) / (self.rotz - self.camera.zpos)) - self.drawx -- using perspective math to calculate draw point on draw plane
    self.drawSizeY = (200 * (self.roty + self.h - self.camera.ypos) / math.abs(self.rotz - self.camera.zpos)) - self.drawy
    
    self.s = self.drawSizeX / self.w -- calculating how big the sprite is compared to the original
